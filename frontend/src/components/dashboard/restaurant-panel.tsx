...
import { UtensilsCrossed, CheckCircle, Plus, Pencil, Trash2 } from "lucide-react";
import { useCompany } from "@/store/company-context";
import { companyUsesRestaurant } from "@/types/company";
...
type OrderRow = {
  ...
  lineItems: { quantity: number; lineTotal: number; itemId: number }[];
};
...
export function RestaurantPanel() {
  ...
  const ok = companyUsesRestaurant(selectedCompanyId);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [iRes, oRes] = await Promise.all([
        fetch(`/api/restaurant-items?${qs}`),
        fetch(`/api/restaurant-orders?${qs}`),
      ]);
      const iJson = await iRes.json();
      const oJson = await oRes.json();
      if (!iRes.ok) throw new Error(iJson.error ?? "Items");
      if (!oRes.ok) throw new Error(oJson.error ?? "Orders");
  ...
  async function createRestaurantItem(name: string) {
  ...
  async function removeOrder(id: number) {
    if (!window.confirm("Delete this restaurant order? This cannot be undone.")) return;
    const qs = new URLSearchParams({ id: String(id), companyId: selectedCompanyId });
    const res = await fetch(`/api/restaurant-orders?${qs}`, { method: "DELETE" });
    const d = await res.json();
    if (!res.ok) {
  ...
  if (!ok) {
    return (
      <div className="space-y-2">
        <h1 className="font-serif text-2xl font-bold text-base-content">Restaurant</h1>
        <p className="text-sm text-base-content/60 max-w-md">
          Switch to Bondo Travellers Hotel to use the restaurant register.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-base-content/60">Service orders and payment status.</p>
        </div>
        <button
          type="button"
      ...
      {items.length === 0 && !loading && (
        <p className="text-sm text-warning">No menu items yet. Search in a line item to create the first one.</p>
      )}
        ...
        <table className="table table-sm">
          <thead className="sticky top-0 z-10 bg-base-200/95 text-xs uppercase text-base-content/70 backdrop-blur">
            <tr>
              ...
              <th className="align-top">Table</th>
      ...
      <DashboardModal open={modal} onClose={() => setModal(false)} title="New restaurant order" wide>
        <form onSubmit={submit} className="space-y-3 max-w-2xl">
          <div className="grid grid-cols-2 gap-2">
            <label className="form-control w-full">
              <span className="label-text text-sm">Table</span>
              <input
      ...
      <DashboardModal
        ...
        title={editing ? `Edit restaurant order #${editing.id}` : "Edit order"}
      >
